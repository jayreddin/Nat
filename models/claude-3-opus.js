 ...
 * with Anthropic's Claude 3 Opus model through the Puter API.
 */

const Claude3OpusModel = {
    name: "Claude 3 Opus",
    provider: "Anthropic",
    description: "Claude 3 Opus is Anthropic's most powerful model, excelling at highly complex tasks requiring deep expertise and careful reasoning.",
    maxTokens: 200000,
    defaultParams: {
        temperature: 0.7,
        top_p: 0.9
    ...
    /**
     * Sends a message to the Claude 3 Opus model
    ...
    async sendMessage(message, options = {}) {
        try {
            ...
            console.log("Sending Claude 3 Opus request with options:", modelOptions);
            ...
            console.log("Claude 3 Opus response:", response);
            
            // If we got an error response, format it properly
            if (response && response.error) {
                throw new Error(response.error.message || "Error from Claude 3 Opus API");
            }
            ...
            if (response.message?.content?.[0]?.text) {
                ...
                console.warn("Unexpected Claude 3 Opus response format:", response);
                return String(response);
            }
        } catch (error) {
            console.error("Error sending message to Claude 3 Opus:", error);
            throw error;
        }
    },
    
    /**
     * Streams a response from the Claude 3 Opus model
     * @param {string|object|array} message - The user message to send to the model
    ...
    async streamMessage(message, onChunk, options = {}) {
        try {
            ...
            console.log("Streaming Claude 3 Opus request with options:", streamOptions);
            console.log("Messages:", formattedMessages);
            ...
            for await (const part of response) {
                console.log("Claude 3 Opus streaming part:", part);
            ...
            console.error("Error streaming message from Claude 3 Opus:", error);
            throw error;
        }
    },
    
    /**
     * Example usage of the Claude 3 Opus model
     ...
     */
    async example(message = "Analyze the implications of artificial general intelligence on society", useStreaming = false) {
        if (useStreaming) {
...
// Export the model
export default Claude3OpusModel; 